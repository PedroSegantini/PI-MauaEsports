document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndDisplayPlayers() {
        try {
            ...
            if (players.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="5" class="text-center">Nenhum jogador cadastrado.</td></tr>';
                return;
            }
            ...
            players.forEach(player => {
                ...
                row.innerHTML = `
                    ...
                    <td>${player.role.charAt(0).toUpperCase() + player.role.slice(1)}</td>
                    <td>
                        <button class="btn btn-sm btn-warning me-2" data-id="${player._id}" data-action="edit">Editar</button>
                        <button class="btn btn-sm btn-danger" data-id="${player._id}" data-action="delete">Excluir</button>
                    </td>
                `;
            ...
            if(error.response && error.response.status === 401) window.location.href = 'login.html';
            tableBody.innerHTML = '<tr><td colspan="5" class="text-center text-danger">Falha ao carregar jogadores.</td></tr>';
        }
    }
    ...
    playerForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const message = error.response?.data?.message || 'Falha ao salvar jogador.';
